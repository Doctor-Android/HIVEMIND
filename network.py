...
class Node:
    ...
    def handle_request(self, request):
        # Example method to handle incoming requests
        host = self.extract_host_from_request(request)
        if host in self.blocked_websites:
            print(f"Access to {host} is blocked.")
            return False  # Block the request
        # Process the request normally
        return True

    def extract_host_from_request(self, request):
